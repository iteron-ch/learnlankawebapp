...
class QuestionbuilderController extends Controller {
    ...
    public function update12(){ die;
     ...
     public function changeQuestionIdData(){ die;
        ini_set('max_execution_time', 0);
        $param['subject'] = 'Math';
        $questions = $this->questionBuilderRepo->getAllQuestionData($param);
       //    asd($questions,0);
        foreach ($questions as $key=>$val){
            if(!empty($val['question_id_post']))
            {
                $question_id_mid = (!empty($val['question_id_mid']))?'.'.$val['question_id_mid']:'';
                $newQuestionId = $val['difficulty'].'.'.$val['strand_reference_code'].'.'.$val['substrand_reference_code'].$question_id_mid.'.'.$val['question_id_post'];
            }
            else{
                $question_id_mid = (!empty($val['question_id_mid']))?'.'.$val['question_id_mid']:'';
                $newQuestionId = $val['difficulty'].'.'.$val['strand_reference_code'].'.'.$val['substrand_reference_code'].$question_id_mid;
            }
            $newQuestionIdPre = $val['difficulty'].'.'.$val['strand_reference_code'].'.'.$val['substrand_reference_code'];
           // echo $key."==>".$val['id']."==".$newQuestionId.'=='.$newQuestionIdPre;
           // echo "<br>";
            
            if(!empty($val['id'])){
                $this->questionBuilderRepo->updateAllQuestionIDData($newQuestionId, $newQuestionIdPre, $val['id']);
            }
        }
    }
}
