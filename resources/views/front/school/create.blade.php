...
<div class="main_container signup_page">
    {!! Form::model($user,['route' => ['frontschool.store'],'files' => true, 'method' => 'post', 'id' =>'addfrm']) !!}
        ...
        <div class="signup_top">
            <div class="signup_bottom">
                <div class="signup_mid">
                    ...
                    <div class="row">
                        <div class="col3">
                            {!! Form::labelControl('school_name',trans('front/school.school_name'))  !!}
                            {!! Form::text('school_name',null,['class'=>'input_text'])  !!}
                            <span class="red">*</span>
                        </div>
                        <div class="col3">
                            {!! Form::labelControl('school_type',trans('front/school.school_type'))  !!}
                            {!! Form::select('school_type', $school_type, null, ['id' => 'school_type', 'class' => 'e1']) !!}   
                            <span class="red">*</span>
                        </div>
                        <div class="col3" id="other-school_type" style="display: {{ Input::old('school_type') == OTHER_VALUE ? 'display' : ( isset($user['school_type']) && $user['school_type'] == OTHER_VALUE ? 'display': 'none')}};">
                            {!! Form::labelControl('school_type',trans('front/front.please_specify'))  !!}
                            {!! Form::text('school_type_other',null,['id' => 'school_type_other','class'=>'input_text']) !!}<span class="red">*</span>
                        </div>
                        <div class="col3">
                    ...
                    <div class="row">
                        <div class="col3">
                            ...
                            {!! Form::text('postal_code',null,['class'=>'input_text']  )  !!}<span class="red">*</span>
                            <div id="suggesstion-box" style="display:none"></div>
                        </div>
                        <div class="col3">
                            {!! Form::labelControl('country',trans('front/front.country'))  !!}
                            {!! Form::select('country', $country,'GB', ['id' => 'country', 'class' => 'e1']) !!}  <span class="red">*</span>
                        </div>
                        <div class="col3">
                    ...
                    <div class="row">
                        <div class="col1">
                            ...
                            {!! Form::textarea('address',null,['class'=>'input_text','cols'=>'','rows'=>''])  !!}<span class="red">*</span>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col3">
                            {!! Form::labelControl('whoyous_id',trans('front/school.whoyous_id'))  !!}
                            {!! Form::select('whoyous_id', $who_you,null, ['id' => 'whoyous_id', 'class' => 'e1']) !!}    <span class="red">*</span>
                        </div>
                        <div class="col3" id="other-whoyous_id" style="display: {{ Input::old('whoyous_id') == OTHER_VALUE ? 'display' : ( isset($user['whoyous_id']) && $user['whoyous_id'] == OTHER_VALUE ? 'display': 'none')}};">
                            {!! Form::labelControl('school_type',trans('front/front.please_specify'))  !!}
                            {!! Form::text('whoyous_other',null,['id' => 'whoyous_other','class'=>'input_text']) !!}<span class="red">*</span>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col3">
                        ...
                        <div class="col3">
                            {!! Form::labelControl('school_name',trans('admin/school.dfe_number'))  !!}
                            {!! Form::text('dfe_number',null,['class'=>'input_text'])  !!} <span class="red">*</span>
                        </div>
                    </div>

                    <div class="head">
                        <div class="col_left">
                            <strong>Additional Number of Pupils</strong>
                            <i><?php echo NO_OF_STUDENTS ?> pupils included. Five Additional Pupils {{ CURRENCY }}{!! $per_five_student_fee !!}</i>
                        </div>
                        <div class="col_mid">
                        ...
                        <div class="col_right mar_t_-13"><?php
                            ...
                            ?>
                            <span>{{ trans('front/front.your_plan') }}</span> <strong>{{ CURRENCY }}{{ $sign_up_fee }}</strong> <br>
                            <span>Total Amount</span> <strong>{{ CURRENCY }}<span id="total_amt_id">{{$sign_up_fee+($noOfStudent*$per_five_student_fee)/5}}</span></strong> 
                        </div>
                    </div>
                ...
                <div class="signup_mid">
                    ...
                    <div class="row">
                        <div class="col1">
                            ...
                            <label for="same_billing" class="css-label">{{ trans('front/front.same_as_basic_information') }}</label>
                            {!! Form::radio('same_billing', 'other', 'checked', ['class' => 'css-radio','id'=>'other']) !!}
                            <label for="other" class="css-label">{{ trans('front/front.other') }}</label>
                    ...
                    <div class="row">
                        <div class="col3">
                            ...
                            {!! Form::select('billing_country', $country,UKCOUNTRYCODE, ['id' => 'billing_country', 'class' => 'e1']) !!}  
                        </div>
                    ...
                    <div class="head signup_2">
                        ...
                        <div class="col_right">
                            ...
                            <strong>{{ CURRENCY }}{{$sign_up_fee}}</strong> 
                        </div>
                        <div class="col_right">
                            <span>TOTAL NO OF PUPILS</span> 
                            <strong><span id="no_of_student_count">{{isset($user['no_of_student'])?(NO_OF_STUDENTS+$user['no_of_student']):'0'}}</span></strong> 
                        </div>
                        <div class="col_right last">
                            <span>{{ trans('front/front.total_amount') }}</span> <?php
                            $noOfStudent = isset($user['no_of_student']) ? $user['no_of_student'] : '0';
                            ?>                          
                            <input type="hidden" name="total_subscription_amount" value="{{$sign_up_fee+($noOfStudent*$per_five_student_fee)/5}}" id="total_subscription_amount">
                            ...
                            <strong>{{ CURRENCY }}<span id="total_payable_amount">{{$sign_up_fee+($noOfStudent*$per_five_student_fee)/5}}</span></strong> 
                        </div>
                    </div>                    
    ...
    <span id="frmStepProcessing" style="display:none;">
        <div class="signup_top">
            <div class="signup_bottom">
                <div class="signup_mid">
                    <div class="signup_header">
                        <div class="point selected" id="btn_basicinfo">1
                            <div class="done"></div>
                            <label for="">{{ trans('front/front.basic_information') }}<span class="arrow"></span></label>
                        </div>
                        <div class="point yellow active">2
...
<script>
            jsMain = new Main();
            var totalNoOfStudents = <?php echo NO_OF_STUDENTS ?>;
            function change_count(act) {
            if (act == 'plus') {
            $("#no_of_student").val($("#no_of_student").val() * 1 + 5);
            ...
            var sign_up_fee = '<?php echo $sign_up_fee ?>';
                    var per_five_student_fee = '<?php echo $per_five_student_fee ?>';
                    var subAmt = (sign_up_fee * 1 + (($("#no_of_student").val() * per_five_student_fee) / 5) * 1).toFixed(2);
    ...
    $('#billing_city_list_id').hide(); }
            ...
            jsMain.toggleOther(['school_type', 'school_type_other'], {{ OTHER_VALUE }});
            jsMain.toggleOther(['whoyous_id', 'whoyous_id_other'], {{ OTHER_VALUE }});
            var error_type = '';
    ...
    $("#js_flash_message").removeClass('space_for_message');
            ...
            $("#no_of_student_count").html(totalNoOfStudents * 1 + $("#no_of_student").val() * 1);
            //$("#total_payable_amount").html(sign_up_fee * 1 + $("#no_of_student").val() * per_five_student_fee * 1)
    ...
    $("#no_of_student_count").html($("#no_of_student").val());
            var sign_up_fee = '<?php echo $sign_up_fee ?>';
            var per_five_student_fee = '<?php echo $per_five_student_fee ?>';
            //$("#total_payable_amount").html(sign_up_fee * 1 + $("#no_of_student").val() * per_five_student_fee * 1)
