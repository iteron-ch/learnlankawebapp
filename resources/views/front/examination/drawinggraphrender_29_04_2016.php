                ...
                <div class="col-xs-12">
                    <div id="dropanswer[%parentIndex%]" class="col-xs-12 margin-top-15 margin-bottom-20" style="width:600px; height:500px;">
                        ...
                        <span ng-repeat="optionele in quesele.option" style="float:left; position:absolute; top:[%optionele.position.top%]; left:[%optionele.position.left%];">
                            <div class="droppable" id="label_[%parentIndex%]_[%$index%]" type="text" style="display: table-cell; float: none;vertical-align: middle; text-align: center; font-weight:bold; width:[%optionele.position.width%]; height:[%optionele.position.height%];">[%renderQuestion[currentQues-1].userresponse[parentIndex][$index].value%]</div>
                        </span>
                    </div>
                </div>
                ...
                <div ng-if="renderQuestion[currentQues - 1].correctAns" class="input_row col-xs-push-1">
                    ...
                    <div class="col-xs-12">
                        <div id="dropanswerCorrectAns[%parentIndex%]" class="col-xs-12 margin-top-15 margin-bottom-20" style="width:600px; height:500px;">
                            <div class="fileinput-preview thumbnail" style="border:1px solid #ffffff; width:600px; height:500px; background-image: url([%backgroundimage[parentIndex]%]);"></div>
                            <span ng-repeat="optionele in quesele.option" style="float:left; position:absolute; top:[%optionele.position.top%]; left:[%optionele.position.left%];">
                                <div class="droppable" id="CorrectAnslabel_[%parentIndex%]_[%$index%]" type="text" style="display: table-cell; float: none;vertical-align: middle; text-align: center; padding-top:20%; font-weight:bold; width:[%optionele.position.width%]; height:[%optionele.position.height%];">[%renderQuestion[currentQues-1].correctAns[parentIndex][$index].value%]</div>
                            </span>
                        </div>
                    </div>
                </div>
