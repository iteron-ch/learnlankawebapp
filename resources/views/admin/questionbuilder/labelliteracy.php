<div class="main_panel" ng-controller="labelliteracyCtrl">
    <form name="labelliteracyForm" id="labelliteracyForm" class="margin-bottom-10">
        <div class="row">
            <div class="col-xs-12">
                ...
                <div class="col-xs-12" ng-repeat="(parentIndex, element) in questionJSON.questions">
                    <accordion-group is-open="isFirstOpen" class="accordion-margin">
                        ...
                        <div class="row" ng-class="{'margin-bottom-10': showAnswerEle && showAnswerEle == (parentIndex + 1)}">
                            ...
                            <div class="row base" ng-repeat="childEle in element.option">
                                <div class="col-xs-12">
                                    <div class="col-xs-10">
                                        <div style="display: inline-block;width: 98%;" >
                                            <textarea ng-hide="true" id="option[%parentIndex + 1%][%$index + 1%]" ng-model="questionJSON.questions[parentIndex].option[$index].option" style="display:none;" name="option[%parentIndex + 1%][%$index + 1%]"></textarea>
                                        </div>
                                    </div>	
                                    <div class="col-xs-1 padding-top-10">
                                        <input ng-hide='true' type="checkbox" ng-disabled="addQuesBtnState" ng-model='questionJSON.questions[parentIndex].option[$index].ischeck'>
                                    </div>
                                    <div class="col-xs-1">
                            ...
                            <div class="row margin-bottom-20">
                                <div class="col-xs-12">
                                    <div class="col-xs-2 col-xs-push-1 radio_demo">
                                        <label>
                                            <input type="radio" ng-disabled="addQuesBtnState" value='drag' ng-model='questionJSON.questions[parentIndex].literacyType'>
                                            [%MASTERS.keywords['dragoptions']%]
                                        </label>
                                    </div>
                                    <div class="col-xs-2 col-xs-push-1 col-xs-push-5 radio_demo">
                                        <label>
                                            <input type="radio" ng-disabled="addQuesBtnState" value='tick' ng-model='questionJSON.questions[parentIndex].literacyType'>
                                            [%MASTERS.keywords['tick(yes/no)']%]
                                        </label>
                                    </div>
                                </div>
                                <div class="col-xs-12" ng-if="questionJSON.questions[parentIndex].literacyType == 'drag'" ng-repeat="ele in questionJSON.questions[parentIndex].answerOption">
                                    <div class="col-xs-4">
                                        <input ng-disabled="addQuesBtnState" style="width:100%; margin-bottom:5px;" id="answeroption[%parentIndex + 1%][%$index + 1%]" ng-model="questionJSON.questions[parentIndex].answerOption[$index].value"/>
                                    </div>	
                                    <div class="col-xs-6">
                                        <span class="glyphicon glyphicon-remove btn-cursor-pointer padding-top-5 remove-option" ng-show="element.answerOption.length > 2 && !addQuesBtnState" tooltip="Remove" ng-click="removeanswerOption(parentIndex, $index)"></span>
                                        <span class="glyphicon glyphicon glyphicon-plus btn-cursor-pointer padding-top-5 add-option" ng-show="element.answerOption.length == $index + 1 && !addQuesBtnState" tooltip="Add" ng-click="addanswerOption(parentIndex)"></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="row" ng-show="showAnswerEle && showAnswerEle == (parentIndex + 1)">
                            ...
                            <div class="row base">						
                                <div class="col-xs-12" ng-if="questionJSON.questions[parentIndex].literacyType == 'drag'">
                                    <div class="col-xs-10 margin-bottom-10 " >
                                        <span class="draggable custom-drag-ele" ng-repeat="ele in questionJSON.questions[parentIndex].answerOption" style="border: 1px solid #000000; padding:5px; margin: 5px;">[%ele.value%]</span>	
                                    </div>
                                </div>		
                                <div class="col-xs-12">
                                    <div class="col-xs-10">
                                        <span style="float:left;position: relative;margin-right:5px;" ng-repeat="childEleA in element.option"  >
                                            ...
                                            <input type="checkbox" ng-model="element.correctAns[$index]" name="answer_[%parentIndex%][%$index%]" id="answer_[%parentIndex%][%$index%]" ng-hide="true" class="custom_multiple" ng-if="childEleA.ischeck && questionJSON.questions[parentIndex].literacyType == 'tick'">
                                            <label ng-if="childEleA.ischeck && questionJSON.questions[parentIndex].literacyType == 'tick'" for="answer_[%parentIndex%][%$index%]"><span></span></label>
                                            <input class="droppable" name="answer_[%parentIndex%][%$index%]" id="answer_[%parentIndex%][%$index%]" type="text" style="width:52px;" ng-if="childEleA.ischeck && questionJSON.questions[parentIndex].literacyType == 'drag'">
                                        </span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row base" style="margin-bottom: 20px;">
                                <div class="col-xs-12">
                                    <div class="col-xs-12">
                                        <button type="button" class="btn btn-default btn-sm pull-right ok-btn-color btn-cursor-pointer" ng-click="submitlabelliteracyCQ( MASTERS.keywords['submitaction1'], MASTERS.keywords['submitaction4'] )">[%MASTERS.keywords['save']%]</button>
                                        <button ng-hide="questionData.id" type="button" class="btn btn-default btn-sm pull-right margin-right-15 ok-btn-color btn-cursor-pointer" ng-click="submitlabelliteracyCQ(MASTERS.keywords['submitaction2'], MASTERS.keywords['submitaction4'])">[%MASTERS.keywords['save&next']%]</button>      
...
<script type="text/ng-template" id="labelliteracyPre">
    ...
    <div class="modal-body">
        ...
        <div class="row margin-bottom-20">
            <div class="col-xs-12 margin-bottom-10" ng-if="quesArray.questions[quesIndex].literacyType == 'drag'">
                <span class="draggable" ng-repeat="ele in quesArray.questions[quesIndex].answerOption" style="border: 1px solid #000000; padding:5px; margin: 5px; z-index: 1;">[%ele.value%]</span>	
            </div>
            <div class="col-xs-12">
                <span style="float:left;position: relative;margin-right:5px; text-align:center" ng-repeat="childEleA in quesArray.questions[quesIndex].option"  >
                    <span ng-class="{'spandecoration': childEleA.ischeck}" ng-bind-html="childEleA.option | to_trusted"></span>
                    <div class="arrow1" ng-if="childEleA.ischeck"></div>
                    <input name="answer_[%parentIndex%][%$index%]" id="answer_[%parentIndex%][%$index%]" ng-hide="true" type="checkbox" class="custom_multiple" ng-if="childEleA.ischeck && quesArray.questions[quesIndex].literacyType == 'tick'">
                    <label ng-if="childEleA.ischeck && quesArray.questions[quesIndex].literacyType == 'tick'" for="answer_[%parentIndex%][%$index%]"><span style="margin: -11px 3px 0 4px; !important"></span></label>
                    <input class="droppable" name="answer_[%parentIndex%][%$index%]" id="answer_[%parentIndex%][%$index%]" type="text" style="width:52px; border:1px solid #2386DB" ng-if="childEleA.ischeck && quesArray.questions[quesIndex].literacyType == 'drag'">
                </span>
            </div>
        </div>
    </div>
