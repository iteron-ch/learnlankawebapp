<div class="main_panel" ng-controller="singlemultipleentryCtrl">
    <form name="singlemultipleentryForm" id="singlemultipleentryForm" class="margin-bottom-10">
        <div class="row">
            <div class="col-xs-12">
                    ...
                    <accordion-group is-open="isFirstOpen" class="accordion-margin">
                                    ...
                                    <div class="col-xs-10">
                                        ...
                                        <span style="margin-left: 20px;">
                                            <label class="radio-inline">
                                                <input type="radio" ng-click="changeMode(parentIndex)" ng-disabled="addQuesBtnState" name="[%'choice'+parentIndex%]" value="single" ng-model="questionJSON.questions[parentIndex].questype"/>[%MASTERS.keywords['singlechoice']%]
                                            </label>
                                            <label class="radio-inline">
                                                <input type="radio" ng-click="changeMode(parentIndex)" ng-disabled="addQuesBtnState" name="[%'choice'+parentIndex%]" value="multiple" ng-model="questionJSON.questions[parentIndex].questype"/>[%MASTERS.keywords['multiplechoice']%]
                                            </label>
                                            <label class="radio-inline">
                                                <input type="radio" ng-click="changeMode(parentIndex)" ng-disabled="addQuesBtnState" name="[%'choice'+parentIndex%]" value="input" ng-model="questionJSON.questions[parentIndex].questype"/>[%MASTERS.keywords['inputchoice']%]
                                            </label>	
                                            <label class="radio-inline">
                                            ...
                                            <tr ng-repeat="(pIndex, subchildele) in element.option track by pIndex">
                                                <td ng-hide="(field.title && !questionJSON.questions[parentIndex].headercol && pIndex == 0) ||  (field.title && !questionJSON.questions[parentIndex].headerrow && $index == 0)" ng-repeat="field in subchildele track by $index">
                                                    <span ng-if="!field.title" class="visibility-editor">
                                                        <input type="checkbox" ng-hide="true" class="custom_[%questionJSON.questions[parentIndex].questype%]" id="checkbox[%parentIndex%][%pIndex%][%$index%]Q" 
                                                               name="checkbox[%parentIndex%][%pIndex%][%$index%]Q" ng-disabled="true"/>
                                                        <label style="display: flex;" for="checkbox[%parentIndex%][%pIndex%][%$index%]Q"><span style="margin-top:5px; margin-right:5px;" ng-disabled="true"></span>
                                                            <!--<input ng-show="questionJSON.questions[parentIndex].questype != 'input'" type="text" ng-disabled="addQuesBtnState" style="width: 86%; font-weight:normal;"  ng-model="field.value"/>-->
                                                            <textarea ng-hide="true" ng-disabled="addQuesBtnState" ng-model="field.value" name="optionText[%parentIndex%][%pIndex%][%$index%]" placeholder="Write your option here"></textarea>
                                                            <input ng-show="questionJSON.questions[parentIndex].questype == 'input'" ng-disabled="questionJSON.questions[parentIndex].questype == 'input' || addQuesBtnState" type="text" style="width: 86%; font-weight:normal;"/>
                                                        </label>		
                                                    </span>
                        ...
                        <div class="row" ng-show="showAnswerEle && showAnswerEle == (parentIndex + 1)">
                                    ...
                                    <div class="col-xs-10">						 
                                        <table>											
                                            <tr ng-repeat="(pIndex, subchildele) in questionJSON.questions[parentIndex].option track by pIndex">
                                                <td ng-hide="(field.title && !questionJSON.questions[parentIndex].headercol && pIndex == 0) ||  (field.title && !questionJSON.questions[parentIndex].headerrow && $index == 0)" ng-repeat="field in subchildele track by $index">
                                                    <span ng-if="!field.title && questionJSON.questions[parentIndex].questype != 'input'">
                                                        <input type="checkbox" ng-hide="true" class="custom_[%questionJSON.questions[parentIndex].questype%]" id="checkbox[%parentIndex%][%pIndex%][%$index%]" 
                                                               name="checkbox[%parentIndex%][%pIndex%][%$index%]" ng-click="updateValues(questionJSON.questions[parentIndex].questype, parentIndex, pIndex, $index)" ng-model="field.checkvalue"/>
                                                        <label for="checkbox[%parentIndex%][%pIndex%][%$index%]" ng-click="updateValues(questionJSON.questions[parentIndex].questype, parentIndex, pIndex, $index)"><span ng-click="updateValues(questionJSON.questions[parentIndex].questype, parentIndex, pIndex, $index)"></span>
                                                            <div style="font-weight:normal; padding-left:2px;" class="pull-right" ng-bind-html="field.value | to_trusted"></div>
                                                        </label>		
                                                    </span>
                                                    <span ng-if="!field.title && questionJSON.questions[parentIndex].questype == 'input'">
                                                            <input ng-show="questionJSON.questions[parentIndex].questype == 'input'" type="text" style="width: auto; font-weight:normal;" ng-model="field.checkvalue" />
                                                    </span>
                            ...
                            <div class="row base" style="margin-bottom: 20px;">
                                <div class="col-xs-12">
                                    <div class="col-xs-12">
                                        <button type="button" class="btn btn-default btn-sm pull-right ok-btn-color btn-cursor-pointer" ng-click="submitsinglemultipleentryCQ( MASTERS.keywords['submitaction1'], MASTERS.keywords['submitaction4'] )">[%MASTERS.keywords['save']%]</button>
                                        <button ng-hide="questionData.id" type="button" class="btn btn-default btn-sm pull-right margin-right-15 ok-btn-color btn-cursor-pointer" ng-click="submitsinglemultipleentryCQ(MASTERS.keywords['submitaction2'], MASTERS.keywords['submitaction4'])">[%MASTERS.keywords['save&next']%]</button>      
...
<script type="text/ng-template" id="singlemultipleentryPre">
                ...
                <table>											
                    <tr ng-repeat="(pIndex, subchildele) in quesArray.questions[quesIndex].option track by pIndex"">
                        <td ng-hide="(field.title && !quesArray.questions[quesIndex].headercol && pIndex == 0) ||  (field.title && !quesArray.questions[quesIndex].headerrow && $index == 0)" ng-repeat="field in subchildele track by $index"">
                            <span ng-if="!field.title">
                                <input type="checkbox" ng-hide="true" class="custom_[%quesArray.questions[quesIndex].questype%]" id="checkbox[%pIndex%][%$index%]" 
                                    ng-click="updateValuestemp( quesArray.questions[quesIndex].questype, parentIndex, pIndex,$index )" name="checkbox[% pIndex %]"/>
                                <label for="checkbox[%pIndex%][%$index%]"><span></span>
                                    <div ng-show="quesArray.questions[quesIndex].questype != 'input'" class="pull-right" style="font-weight:normal; padding-left:2px;" ng-bind-html="field.value | to_trusted"></div>
                                    <input ng-show="quesArray.questions[quesIndex].questype == 'input'" type="text" style="width: auto; font-weight:normal;" />
                                </label>		
                            </span>
                            <span ng-if="field.title">
                                <div style="font-weight:normal; padding-left:2px;" ng-bind-html="field.value | to_trusted"></div>
                    ...
                    <tr ng-show="false">
                        <td class="remove-tr-border" ng-repeat="field in quesArray.questions[quesIndex].option[0]">
                            <a ng-hide="quesArray.questions[quesIndex].option[0].length < 4" ng-if="$index != 0">
                                <span class="glyphicon glyphicon-remove btn-cursor-pointer btn-font-18 pull-right" tooltip="Remove" style="padding-top: 2px;padding-right: 10px;"></span>
                            </a>
                        </td>
                    </tr>
