<div class="main_panel" ng-controller="drawinggraphsCtrl">
    <form name="drawinggraphsForm" id="drawinggraphsForm" class="margin-bottom-10">
        <div class="row">
            <div class="col-xs-12">
                    ...
                    <accordion-group is-open="isFirstOpen" class="accordion-margin">
                        ...
                        <div class="row" ng-class="{'margin-bottom-10': showAnswerEle && showAnswerEle == (parentIndex + 1)}">
                            ...
                            <div class="row base margin-bottom-20">
                                <div class="col-xs-12 margin-bottom-20">
                                    <button type="button" class="btn btn-default btn-sm margin-left-15 ok-btn-color btn-cursor-pointer" ng-click="dropContainerAdd(parentIndex)">[%MASTERS.keywords['container']%]</button>		
                                    <button type="button" class="btn btn-default btn-sm delete-btn btn-cursor-pointer" ng-click="dropContainerRemove(parentIndex)">[%MASTERS.keywords['deletecontainer']%]</button>				
                                </div>
                                <div class="col-xs-12 margin-bottom-20">
                                    <div class="col-xs-10 margin-bottom-20" id="quesDrag[%parentIndex%]">
                                        <div class="fileinput"  id="fileinput[%parentIndex%]" ng-class="questionJSON.id ? 'fileinput-exists' : 'fileinput-new'" data-provides="fileinput" style="width: 600px; height: 500px;">
                                            ...
                                            <span class="draggable" id="option_[%parentIndex%]_[%$index%]" ng-repeat="optionele in questionJSON.questions[parentIndex].option" style="float:left; z-index:1; padding-left:12px; position:absolute; top:[%optionele.position.top%]; left:[%optionele.position.left%];">
                                                <span class="icon" style="float: left; margin: 1px; position:absolute; left:0px; top:px;">*</span>
                                                <div type="text" class="resizable" id="[%optionele.label%]" class="ui-state-active" style="float:left; opacity:0.5; border:1px solid red; width:[%optionele.position.width%]; height:[%optionele.position.height%];">[%optionele.label%]</div>
                                            </span>
                                ...
                                <div class="col-xs-12 margin-bottom-20">
                                    <div class="col-xs-12" ng-repeat="eledragoption in questionJSON.questions[parentIndex].option">
                                        <div class="col-xs-2">
                                            <!--[%eledragoption.label%]-->Option [%$index+1%]
                                        </div>
                                        <div class="col-xs-2 col-xs-pull-1 margin-bottom-10">
                                            <input type="text" ng-disabled="addQuesBtnState" id="[%'labelname'+ parentIndex + $index%]" ng-model="questionJSON.questions[parentIndex].option[$index].value" class="btn-input-style text-center" />
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="row" ng-show="showAnswerEle && showAnswerEle == (parentIndex + 1)">
                            ...
                            <div class="row base">
                                <div class="col-xs-12">
                                    <div id="setanswer[%parentIndex%]" class="col-xs-12 margin-top-15 margin-bottom-20">
                                        <span  class="draggable" style="border: 1px solid #000000; margin:10px; background-color:#edf6ff; padding: 5px 10px 8px 10px;" ng-repeat="dragele in questionJSON.questions[parentIndex].option">[%dragele.value%]</span>
                                    </div>
                                </div>
                                <div class="col-xs-12">
                                    <div id="dropanswer[%parentIndex%]" class="col-xs-12 margin-top-15 margin-bottom-20" style="width: 600px; height: 500px;">
                                        <div class="fileinput-preview thumbnail" style="border:1px solid #ffffff; width:600px; height:500px; background-image: url([%'/questionimg/'+ questionJSON.questions[parentIndex].imgPath%]);"></div>
                                        <span ng-repeat="optionele in questionJSON.questions[parentIndex].option" style="float:left; position:absolute; top:[%optionele.position.top%]; left:[%optionele.position.left%];">
                                            <div class="droppable" id="label_[%parentIndex%]_[%$index%]" type="text" style="float:left; text-align: center; font-weight:bold; width:[%optionele.position.width%]; height:[%optionele.position.height%];">[%questionJSON.questions[parentIndex].correctAns[$index].value%]</div>
                                        </span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row base" style="margin-bottom: 20px;">
                                <div class="col-xs-12">
                                    <div class="col-xs-12">
                                        <button type="button" class="btn btn-default btn-sm pull-right ok-btn-color btn-cursor-pointer" ng-click="submitdrawinggraphsCQ( MASTERS.keywords['submitaction1'], MASTERS.keywords['submitaction4'] )">[%MASTERS.keywords['save']%]</button>
                                        <button ng-hide="questionData.id" type="button" class="btn btn-default btn-sm pull-right margin-right-15 ok-btn-color btn-cursor-pointer" ng-click="submitdrawinggraphsCQ(MASTERS.keywords['submitaction2'], MASTERS.keywords['submitaction4'])">[%MASTERS.keywords['save&next']%]</button>      
...
<script type="text/ng-template" id="drawinggraphsPre">
    ...
    <div class="row">
        ...
        <div class="col-xs-12">
            <div id="setpreanswer" class="col-xs-12 margin-top-15 margin-bottom-20">
                <span  class="draggable" style="border: 1px solid #000000; margin:10px; background-color:#edf6ff; padding: 5px 10px 8px 10px;" ng-repeat="dragele in quesArray.questions[quesIndex].option">[%dragele.value%]</span>
            </div>
        </div>
        <div class="col-xs-12">
            <div id="droppreanswer" class="col-xs-12 margin-top-15 margin-bottom-20" style="width: 600px; height: 500px;">
                <div class="fileinput-preview thumbnail" style="border:1px solid #ffffff; width:600px; height:500px; background-image: url([%backgroundimage%]);"></div>
                <span ng-repeat="optionele in quesArray.questions[quesIndex].option" style="float:left; position:absolute; top:[%optionele.position.top%]; left:[%optionele.position.left%];">
                    <div class="droppable" id="label_[%quesIndex%]_[%$index%]" type="text" style="display: table-cell; float: none;vertical-align: middle; text-align:center; font-weight:bold; width:[%optionele.position.width%]; height:[%optionele.position.height%];"></div>
                </span>
            </div>
        </div>
