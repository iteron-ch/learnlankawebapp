/**
 * Created by sunny.rana on 07-09-2015.
 */
'use strict';

/**
 * @ngdoc function
 * @name questionBuilder.controller:singlemultipleentryCtrl
 * @description
 * # singlemultipleentryCtrl
 * Controller of the questionBuilder
 */
var app = angular.module('questionBuilder');
app.controller( 'singlemultipleentryCtrl' , function ( $scope, $window, $sce, $rootScope, $routeParams, $modal, Flash, $http ) {
            ...
            questions: [
                {
                    ...
                    option: [
                        ...
                        [{"value": "", "checkvalue": '', "title": true}, {"value": "", "checkvalue": '', "title": false}, {"value": "", "checkvalue": '', "title": false}, {"value": "", "checkvalue": '', "title": false}],
                        [{"value": "", "checkvalue": '', "title": true}, {"value": "", "checkvalue": '', "title": false}, {"value": "", "checkvalue": '', "title": false}, {"value": "", "checkvalue": '', "title": false}],
                        [{"value": "", "checkvalue": '', "title": true}, {"value": "", "checkvalue": '', "title": false}, {"value": "", "checkvalue": '', "title": false}, {"value": "", "checkvalue": '', "title": false}]
                    ],
                    correctAns: '0',
    ...
    $scope.submitsinglemultipleentryCQ = function ( navigateTo, actionMode ) {
        ...
        if ($scope.validatedVal) {
            ...
            if ($scope.questionJSON.questions[index].correctmark.length === 1) {
                angular.forEach($scope.questionJSON.questions[index].correctmark, function (childEle, childindex) {
                    $scope.questionJSON.questions[index].correctmark[childindex].val = $scope.questionJSON.questions[index].option.length - 1;
    ...
    $scope.previewQuestion = function (preQuesIndex, popupSize) {
        ...
        if ($scope.validatedVal) {
            ...
            var modalInstance = $modal.open({
                templateUrl: 'singlemultipleentryPre',
                controller: 'singlemultipleentryPreCtrl',
                size: popupSize,
                scope: $scope,
                resolve: {
    ...
    $scope.changeMode = function( quesIndex ){
        if( $scope.questionJSON.questions[quesIndex].questype === 'input' ){
            $('#questionId'+quesIndex).find('.visibility-editor').find('.cke_textarea_inline').css('display', 'none');
            angular.forEach($scope.questionJSON.questions[quesIndex].option, function (oEle, oIndex) {
                angular.forEach(oEle, function (oEleChild, oIndexChild) {
                    $scope.questionJSON.questions[quesIndex].option[oIndex][oIndexChild].checkvalue = '';
                });
            });
        } else {
            $('#questionId'+quesIndex).find('.visibility-editor').find('.cke_textarea_inline').css('display', 'block');
        }
    };
    ...
    $scope.addColumn = function (index) {
        ...
        angular.forEach($scope.questionJSON.questions[index].option, function (oEle, oIndex) {
            if (oIndex === 0) {
                ...
                $scope.questionJSON.questions[index].option[oIndex].push({"value": "", "checkvalue": '', "title": false});
            }
            angular.forEach(oEle, function (oEleChild, oIndexChild) {
                var optionName = "optionText" + index + oIndex + oIndexChild;
    ...
    $scope.addRow = function (index) {
        ...
        angular.forEach($scope.questionJSON.questions[index].option[0], function (childEle, childindex) {
            if (childindex === 0) {
                ...
                tempArray.push({"value": "", "checkvalue": '', "title": false});
            }
        });
    ...
    $scope.updateValues = function (type, quesIndex, parentInd, childInd) {
        if (type === 'single') {
            angular.forEach($scope.questionJSON.questions[quesIndex].option[parentInd], function (childEle, index) {
                if ( index === childInd ) {
                    $scope.questionJSON.questions[quesIndex].option[parentInd][index].checkvalue = true;
                } else {
                    $scope.questionJSON.questions[quesIndex].option[parentInd][index].checkvalue = false;
                }
    ...
    $scope.addCorrectmark = function (parentIndex) {
        if ($scope.questionJSON.questions[parentIndex].option.length - 1 > $scope.questionJSON.questions[parentIndex].correctmark.length) {
...
app.controller('singlemultipleentryPreCtrl', function ( $scope, $modalInstance, questionPre, $http, $modal ) {

    $scope.quesIndex = questionPre.questionindex;
    $scope.quesArray = questionPre.assignment;
    $scope.updateValuestemp = function (type, quesIndex, parentInd, childInd) {
        if (type === 'single') {
            angular.forEach( $scope.quesArray.questions[quesIndex].option[parentInd], function ( childEle, index ) {
                if (index === childInd) {
                    $scope.quesArray.questions[quesIndex].option[parentInd][index].checkvalue = true;
                } else {
                    $scope.quesArray.questions[quesIndex].option[parentInd][index].checkvalue = false;
                }
