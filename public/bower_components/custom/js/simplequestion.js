/**
 * Created by sunny.rana on 25-08-2015.
 */
'use strict';

/**
 * @ngdoc function
 * @name questionBuilder.controller:simplequesCtrl
 * @description
 * # simplequesCtrl
...
app.controller( 'simplequesCtrl' , function ( $scope, $window, $sce, $rootScope, $routeParams, $modal, Flash, $http ) {
    ...
    /*Add here else answer in the question*/
    $scope.addElseAnswer = function (quesIndex, answerIndex) {
        $scope.questionJSON.questions[quesIndex].correctAns.push({value: ''});
    };

    /*Remove here else answer in the question*/
    $scope.removeElseAnswer = function (quesIndex, answerIndex) {
        $scope.questionJSON.questions[quesIndex].correctAns.splice(answerIndex, 1);
    };
    ...
    $scope.submitValidateQues = function(){
        ...
        angular.forEach($scope.questionJSON.questions, function (ele, questionIndex) {
            ...
            if ( $scope.questionJSON.questions[questionIndex].correctAns.length === 0 || $('#mark' + questionIndex).val()==='' || parseInt($('#mark' + questionIndex).val())===0 ) {
    ...
    $scope.submitsimpleQues = function ( navigateTo, actionMode ) {
    ...
    $scope.previewQuestion = function (preQuesIndex, popupSize) {
        ...
        if ($scope.validatedVal) {
            ...
            var modalInstance = $modal.open({
                templateUrl: 'simpleQuestionPre',
                controller: 'simpleQuesPreCtrl',
                size: popupSize,
                scope: $scope,
                resolve: {
...
app.controller('simpleQuesPreCtrl', function ( $scope, $modalInstance, questionPre, $http, $modal ) {
