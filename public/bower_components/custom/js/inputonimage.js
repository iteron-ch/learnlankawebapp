/**
 * Created by sunny.rana on 28-12-2015.
 */
'use strict';

/**
 * @ngdoc function
 * @name questionBuilder.controller:inputonimageCtrl
 * @description
 * # inputonimageCtrl
 * Controller of the questionBuilder
 */
var app = angular.module('questionBuilder');
app.controller( 'inputonimageCtrl' , function ( $scope, $window, $sce, $rootScope, $routeParams, $modal, Flash, $http, $timeout ) {
            ...
            questions: [
                {
                    ques: '',
                    imgPath:'',
                    option: [{label:'Label 1',value:'', position:{top: '0px', left: '15px', width: '50px', height: '20px'}}],
                    correctAns: [[{value : ''}, {value : ''}, {value : ''}]],
    ...
    $scope.questionAdd = function () {
        $scope.questionJSON.questions.push({
            ques: '',
            imgPath:'',
            option: [{label:'Label 1', position:{top: '0px', left: '15px', width: '50px', height: '20px'}}],
            correctAns: [[{value : ''}, {value : ''}, {value : ''}]],
            correctmark: [{val: '', marks: ''}],
            mark: 1,
            questiontype: $scope.questionCode
    ...
    $scope.inputBoxAdd = function( parentIndex ){
        ...
        $scope.questionJSON.questions[parentIndex].option.push({label:'Label '+($scope.questionJSON.questions[parentIndex].option.length + 1), position:{top: '0px', left: leftCalc, width: '50px', height: '20px'}});
        $scope.questionJSON.questions[parentIndex].correctAns.push([{value : ''}, {value : ''}, {value : ''}]);
        $scope.responsiveEle();
    };
    
    $scope.inputBoxDelete = function( parentIndex ){
    ...
    $scope.submitinputonimageCQ = function ( navigateTo, actionMode ) {
        ...
        if ($scope.validatedVal) {
            ...
            if ($scope.questionJSON.questions[index].correctmark.length === 1 && $scope.questionJSON.questions[index].correctmark[0].val === '') {
                angular.forEach($scope.questionJSON.questions[index].correctmark, function (childEle, childindex) {
                    $scope.questionJSON.questions[index].correctmark[childindex].val = $scope.questionJSON.questions[index].correctAns.length*3;
    ...
    $scope.addCorrectmark = function (parentIndex) {
        if (($scope.questionJSON.questions[parentIndex].correctAns.length*3) > $scope.questionJSON.questions[parentIndex].correctmark.length) {
            var tempVar = $scope.questionJSON.questions[parentIndex].correctmark;
            var valueInput = tempVar[tempVar.length - 1].val ? tempVar[tempVar.length - 1].val - 1 : (($scope.questionJSON.questions[parentIndex].correctAns.length*3) - 1);
            var marksInput = tempVar[tempVar.length - 1].marks ? tempVar[(tempVar.length) - 1].marks - 1 : (parseInt($('#mark' + parentIndex).val(), 10) - 1);
            if ( valueInput === 0 ) {
                valueInput = $scope.questionJSON.questions[parentIndex].correctAns.length*3;
            }
            if ( marksInput === 0 ) {
            ...
            var message = '<strong>'+$rootScope.MASTERS.keywords['typeerror']+'</strong> Max ' + ($scope.questionJSON.questions[parentIndex].correctAns.length*3) + ' option allowed.';
            Flash.create('danger', message);
        }
    ...
    $scope.previewQuestion = function (preQuesIndex, popupSize) {
        ...
        if ($scope.validatedVal) {
            ...
            var modalInstance = $modal.open({
                templateUrl: 'inputonimagePre',
                controller: 'inputonimagePreCtrl',
                size: popupSize,
                scope: $scope,
                resolve: {
...
app.controller('inputonimagePreCtrl', function ( $scope, $modalInstance, questionPre, $http, $timeout, $modal ) {

    $scope.quesIndex = questionPre.questionindex;
    $scope.quesArray = questionPre.assignment;
    $scope.backgroundimage = '/questionimg/'+ $scope.quesArray.questions[$scope.quesIndex].imgPath;
