...
app.controller('drawlineonimageCtrl', function($scope, $rootScope, $timeout) {
 ...
 $scope.preCanvasRender = function() {
        var canvas = new fabric.Canvas('q_preCanvas_' + $rootScope.MASTERS.questions[$scope.setIndex].id + '_' + $scope.quesIndex, {selection: false});
        var canvas1 = new fabric.Canvas('q_preCanvasImg_' + $rootScope.MASTERS.questions[$scope.setIndex].id + '_' + $scope.quesIndex);
        var line, isDown, imageUrl;
...
app.controller('measurementCtrl', function($scope, $rootScope, $timeout) {
 ...
 /*Set canvas data here*/
    $scope.setCanvasDataPreQ = function(src, width, height) {
        var canvas = document.getElementById('q_measurementCanvasPre_' + $rootScope.MASTERS.questions[$scope.setIndex].id + '_' + $scope.quesIndex);
        $scope.stopdragging = false;
        ...
        function start() {
            var element = '';
            element = $('q_measurementCanvasPre_' + $rootScope.MASTERS.questions[$scope.setIndex].id + '_' + $scope.quesIndex);
    ...
    $timeout(function() {
        $scope.setCanvasDataPreQ($rootScope.MASTERS.questions[$scope.setIndex].questions[$scope.quesIndex].imgPath, $scope.canvasWidth, $scope.canvasHeigth);
