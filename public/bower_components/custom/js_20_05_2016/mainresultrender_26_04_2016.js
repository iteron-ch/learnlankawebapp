...
app.controller('renderCtrl', function ( $scope, $rootScope, $location, $filter, $sce, $timeout, $route, $modal, $interval, $http ) {
    ...
    $scope.navigateToQues = function( action, curQues ){
        var curQues1 = curQues;
        $rootScope.$broadcast("SUBMIT_RESPONSE");        
        
        if($rootScope.renderQuestion.length == curQues) // 
            curQues1 = curQues*1-1;
        
        if(action === 'next' || action === 'link'){
            if( action === 'link' &&  $rootScope.renderQuestion[curQues1].attempstatus === 'new' ){
            } else {
...
app.controller('matchdragdropRenderCtrl', function ($scope, $serverRequest, $timeout, $rootScope) {
	...
	$scope.rightPersons[index] = [];
        $scope.leftPersonscorrectAns[index] = [];
	$scope.rightPersonscorrectAns[index] = [];
        angular.forEach(ele.option, function(ele, childIndex){
            ...
            $scope.leftPersonscorrectAns[index].push({id: index,name: ele.left,cls: 'draggable'});
            $scope.rightPersonscorrectAns[index].push({id: index,name: ele.right,cls: 'droppable'});
	}); 
    });
    ...
    if($rootScope.renderQuestion[$scope.currentQues-1].correctAns){
        $timeout(function(){
            angular.forEach($rootScope.renderQuestion[$scope.currentQues-1].questions, function (quesEle, quesIndex) {
                console.log( "calling initialize...correctAns" );
                $scope.initializecorrectAns(quesIndex, 'edit');
            });  
        },1000);
    }
    ...
    $scope.initializecorrectAns = function( parentIndex, mode ){
        // set up the drawing area from Body of the document
        console.log("calling function..");
        myLinescorrectAns[parentIndex] = [];
        $("#correctAnssvgbasics"+parentIndex).css("height", $("#correctAnspnlAllIn"+parentIndex).height()).css("width", $("#correctAnspnlAllIn"+parentIndex).width());
	...
	if( $('#correctAnssvgbasics'+parentIndex).find('svg').length ){
            $('#correctAnssvgbasics'+parentIndex).find('svg').remove();
        }
        svgcorrectAns[parentIndex]= Raphael("correctAnssvgbasics"+parentIndex, $("#correctAnssvgbasics"+parentIndex).width(), $("#correctAnssvgbasics"+parentIndex).height());
        if( mode === 'edit'){
            angular.forEach( $rootScope.renderQuestion[$scope.currentQues-1].questions[parentIndex].option, function (optionEle, optionIndex) {        
                var userresponsetext = $rootScope.renderQuestion[$scope.currentQues-1].correctAns[parentIndex][optionIndex];
                if( userresponsetext.source && userresponsetext.target ){
                    var sourceValue = $('#correctAnsl_'+userresponsetext.source+'_'+parentIndex);
                    var targetValue = $('#correctAnsr_'+userresponsetext.target+'_'+parentIndex);
                    sourceValue.find("input:hidden").val(userresponsetext.source + "_" + userresponsetext.target);
                    svgDrawLinecorrectAns(targetValue, sourceValue, parentIndex);
                }
            });
        }
    };
    ...
    if($rootScope.renderQuestion[$scope.currentQues-1].correctAns){  
        $timeout(function(){
            angular.forEach($rootScope.renderQuestion[$scope.currentQues-1].questions, function (quesEle, quesIndex) {
                ...
                if( $(objQues).find('input').length ){ 
                    angular.forEach($(objQues).find('input'), function ( ele, index ) {
                        $($('#correctAns'+quesIndex).find('input')[index]).val( $rootScope.renderQuestion[$scope.currentQues-1].correctAns[quesIndex][index].value );
                    });
                }
            });
        })    
...
app.controller('insertliteracyRenderCtrl', function ( $scope, $serverRequest, $timeout, $rootScope ) {
    ...
    if($rootScope.renderQuestion[$scope.currentQues-1].correctAns){
        $timeout(function(){
            angular.forEach($rootScope.renderQuestion[$scope.currentQues-1].questions, function (quesEle, quesIndex) {
                angular.forEach($('#textEditorCorrectAns'+quesIndex).find('input[name^="qus_ans'+quesIndex+'"]'), function (ele, indexChild) {
                    $($('#textEditorCorrectAns'+quesIndex).find('input[name^="qus_ans'+quesIndex+'"]')[indexChild]).val($rootScope.renderQuestion[$scope.currentQues-1].correctAns[quesIndex][indexChild]);            
                });
...
app.controller('dragdropRenderCtrl', function ( $scope, $serverRequest, $timeout, $rootScope ) {    
    ...
    $scope.bindDropObject = function( questionIndex, opIndex, isCorrectAnsRender ){ 
        var ansobject = isCorrectAnsRender ? $rootScope.renderQuestion[$scope.currentQues-1].correctAns : $rootScope.renderQuestion[$scope.currentQues-1].userresponse
        if ( ansobject[questionIndex].length ){
            return ansobject[questionIndex][opIndex];
        } else {
            return '';
        }
    };
    
    $scope.getActiveClass = function( questionIndex, opIndex, isCorrectAnsRender ){
        var ansobject = isCorrectAnsRender ? $rootScope.renderQuestion[$scope.currentQues-1].correctAns : $rootScope.renderQuestion[$scope.currentQues-1].userresponse
        if ( ansobject[questionIndex].length ){
            if( ansobject[questionIndex][opIndex] ){
    ...
    $scope.bindDropObjectSortable = function( questionIndex, optionIndex, isCorrectAnsRender ){
        var ansobject = isCorrectAnsRender ? $rootScope.renderQuestion[$scope.currentQues-1].correctAns : $rootScope.renderQuestion[$scope.currentQues-1].userresponse
        if ( ansobject[questionIndex].length ){
            return ansobject[questionIndex][optionIndex];
        } else {
...
app.controller('numberwordselectRenderCtrl', function ( $scope, $serverRequest, $timeout, $rootScope ) {
    ...
    if(!$rootScope.renderQuestion[$scope.currentQues-1].userresponse){    
        ...
        $timeout(function(){
            angular.forEach( $rootScope.renderQuestion[$scope.currentQues-1].questions, function (quesEle, quesIndex) {
                var newArr = [];
                $.each($rootScope.renderQuestion[$scope.currentQues-1].userresponse[quesIndex],function(i,val){
                    newArr.push($("<div/>").html(val).text())
                });
                angular.forEach( quesEle.option.optionvalue, function (optionEle, optionIndex) {
                    if($.inArray($("<div/>").html(optionEle.value).text(), newArr) !== -1){
    ...
    if($rootScope.renderQuestion[$scope.currentQues-1].correctAns){
        $timeout(function(){
            angular.forEach( $rootScope.renderQuestion[$scope.currentQues-1].questions, function (quesEle, quesIndex) {
                var newArr = [];
                $.each($rootScope.renderQuestion[$scope.currentQues-1].correctAns[quesIndex],function(i,val){
                    newArr.push($("<div/>").html(val).text())
                });
                angular.forEach( quesEle.option.optionvalue, function (optionEle, optionIndex) {         
                    if($.inArray($("<div/>").html(optionEle.value).text(), newArr) !== -1){
                        $('#correctAns'+quesIndex+'_'+optionIndex).addClass($rootScope.renderQuestion[$scope.currentQues-1].questions[quesIndex].option.draw + '-cls');
                    }
                });
            });
        })
    ...
    $timeout(function(){
        ...
        $("[name='addDrawClass']").unbind().bind('click', function () {
            ...
            $('#questionID'+quesId).find("[name='addDrawClass']").each(function( index ) {
                if( $(this).hasClass('circle-cls') || $(this).hasClass('underline-cls') ){
        ...
        if($rootScope.renderQuestion[$scope.currentQues-1].correctAns){
            $timeout(function(){
                angular.forEach($rootScope.renderQuestion[$scope.currentQues-1].questions, function (ele, questionIndex) {
                    ...
                    angular.forEach($rootScope.renderQuestion[$scope.currentQues-1].questions[questionIndex].option, function (childEle, childindex) {
                        if ($rootScope.renderQuestion[$scope.currentQues-1].questions[questionIndex].option[childindex].ischeck) {
                            var answerID = 'Correctanswer_' + questionIndex + childindex;
                            $('#'+answerID).val( $rootScope.renderQuestion[$scope.currentQues-1].correctAns[questionIndex][tempindex] );
                            tempindex = tempindex + 1;
                        }
                    });
                });
...
app.controller('drawlineonimageRenderCtrl', function ( $scope, $serverRequest, $timeout, $rootScope ) {
    ...
    $scope.setanswerCanvasRenderCorrectAns = function( quesId ){
        var canvas = new fabric.Canvas('setansCanvasCorrectAns'+quesId, { selection: false });
	var canvas1 = new fabric.Canvas('setansCanvasImgCorrectAns'+quesId);
	var line, isDown, imageUrl, isMove;
        var correctpoints, correctset;
        imageUrl = "/questionimg/"+$rootScope.renderQuestion[$scope.currentQues-1].questions[quesId].imgPath;
	fabric.Image.fromURL(imageUrl, function(oImg) {
	  canvas1.add(oImg);
        ...
        if( $rootScope.renderQuestion[$scope.currentQues-1].correctAns[quesId].length ){
            angular.forEach($rootScope.renderQuestion[$scope.currentQues-1].correctAns[quesId], function (uoption, iuoption) {
                line = new fabric.Line(uoption.points, {
                    strokeWidth: 5,
                    fill: 'red',
                    stroke: 'red',
    ...
    $timeout(function(){
        ...
        angular.forEach($rootScope.renderQuestion[$scope.currentQues-1].questions, function (quesEle, quesIndex) {    
             ...
             $scope.setanswerCanvasRenderCorrectAns(quesIndex);
        });        
    });    
});
...
app.controller('measurementRenderCtrl', function ( $scope, $serverRequest, $timeout, $rootScope, $sce ) {
    ...
    $scope.setCanvasData = function (src, width, height, quesIndex,srcObj) {

        var canvas = document.getElementById(srcObj+quesIndex);
        $scope.stopdragging = true;
        ...
        if(srcObj == 'measurementCanvasAns'){
            var img1 = new Image
            img1.src = '/questionimg/'+src;
            img1.height = $scope.canvasHeight+'px';
            img1.width = $scope.canvasWidth+'px';
            img1.onload = function(){
                ctx.drawImage(img1, 0, 0, canvas.height, canvas.width);
        ...
        function updateLine1() { 
            ...
            if( $rootScope.renderQuestion[$scope.currentQues-1].questions[quesIndex].view === 'vertical' ){
                var x = $rootScope.renderQuestion[$scope.currentQues-1].correctAns[quesIndex];
                var y = 0;
//                $rootScope.renderQuestion[$scope.currentQues-1].correctAns[quesIndex] = x;
                line.x1 = x;
                line.y1 = 0;
                line.x2 = x;
                ...
                var x = 0;
                var y = $rootScope.renderQuestion[$scope.currentQues-1].correctAns[quesIndex];                
  //              $rootScope.renderQuestion[$scope.currentQues-1].correctAns[quesIndex] = y;
                line.x1 = 0;
                line.y1 = y;
    ...
    $timeout(function(){
        ...
        angular.forEach($rootScope.renderQuestion[$scope.currentQues-1].questions, function (quesEle, quesIndex) { 
            setTimeout(function(){
                $scope.setCanvasData(quesEle.imgPath, $scope.canvasWidth, $scope.canvasHeigth, quesIndex,'measurementCanvas' );
                $scope.setCanvasData(quesEle.imgPath, $scope.canvasWidth, $scope.canvasHeigth, quesIndex,'measurementCanvasAns' );
            })            
        });
    });    
});
    ...
    $timeout(function(){
        ...
        angular.forEach($rootScope.renderQuestion[$scope.currentQues-1].questions, function (quesEle, quesIndex) { 
           ...
           var forDotsjoinCorrectAns = {
                'ques': $rootScope.renderQuestion[$scope.currentQues-1].questions[quesIndex],
                'userresponse' : $rootScope.renderQuestion[$scope.currentQues-1].correctAns[quesIndex],
                'action': 'student'
            }
            var lockCorrectAns = new DotsJoin("#patternContainerCorrectAns"+quesIndex, forDotsjoinCorrectAns );
        });
        ...
        $( window ).on( "watchdots", function( event, obj ) {
            ...
            $rootScope.renderQuestion[$scope.currentQues-1].correctAns[obj.holder[0].id.split('Container')[1]].push( obj.pattern );
        });
    });    
});
...
app.controller('reflectionRenderCtrl', function ( $scope, $serverRequest, $timeout, $rootScope ) {
    ...
    if($rootScope.renderQuestion[$scope.currentQues-1].correctAns){
        angular.forEach($rootScope.renderQuestion[$scope.currentQues-1].questions, function (quesEle, quesIndex) {
            $rootScope.renderQuestion[$scope.currentQues-1].questions[quesIndex].ans_correct = $rootScope.renderQuestion[$scope.currentQues-1].correctAns[quesIndex];
        });
    }
    ...
    $timeout(function(){
        ...
        angular.forEach($rootScope.renderQuestion[$scope.currentQues-1].questions, function (quesEle, quesIndex) {    
            ...
            if($rootScope.renderQuestion[$scope.currentQues-1].correctAns){
                var lockcorrectAns = new Reflection("#correctAnssetpatternContainer_ques_"+quesIndex, forDotsjoin );
            }
            forDotsjoin.action = 'ans';
            forDotsjoin.addevent = true;
            var lock = new Reflection("#setpatternAnswer_ans_"+quesIndex, forDotsjoin );
            
            forDotsjoin.action = 'ans_correct';
            forDotsjoin.addevent = false;
            if($rootScope.renderQuestion[$scope.currentQues-1].correctAns){
                var lockcorrectAns1 = new Reflection("#correctAnssetpatternAnswer_ans_"+quesIndex, forDotsjoin );
            }
...
app.controller('piechartRenderCtrl', function ( $scope, $serverRequest, $timeout, $rootScope ) {
    ...
    if($rootScope.renderQuestion[$scope.currentQues-1].correctAns){  
        angular.forEach($rootScope.renderQuestion[$scope.currentQues-1].questions, function (quesEle, quesIndex) {
            $scope.percentrCorrectAns[quesIndex] = $rootScope.renderQuestion[$scope.currentQues-1].correctAns[quesIndex][0].value;
        }); 
    }
...
app.controller('symmetricRenderCtrl', function ( $scope, $serverRequest, $timeout, $rootScope, $sce ) {
    ...
    $timeout(function(){
        ...
        angular.forEach($rootScope.renderQuestion[$scope.currentQues-1].questions, function (quesEle, quesIndex) {             
            ...
            if($rootScope.renderQuestion[$scope.currentQues-1].correctAns){  
                var lockCorrectAns = new Reflection("#patternContainerCorrectAns"+quesIndex, forDotsjoin );    
            }
...
app.controller('inputonimageRenderCtrl', function ( $scope, $serverRequest, $timeout, $rootScope ) {
    ...
    $timeout(function(){
        angular.forEach($rootScope.renderQuestion[$scope.currentQues-1].questions, function (quesEle, quesIndex) {
            $scope.backgroundimage[quesIndex] = '/questionimg/'+ quesEle.imgPath;
            $scope.position[quesIndex] = [];
            angular.forEach(quesEle.option, function (optionEle, optionIndex) {        
                $scope.position[quesIndex].push({top:optionEle.position.top, left:optionEle.position.left });
            });
        }); 
    },2000);
    
    $scope.bindBackGroundImg = function( imaPath ){
        return 'background-image: url(/questionimg/'+ imaPath+'); background-repeat: no-repeat';
    };
...
app.service( '$serverRequest', function ( $rootScope, $http ) {
    ...
    $serverRequest.postData = {
        ...
        saveUserResponse: function ( json ){
            ...
            $rootScope.quessummary.remaining = $rootScope.quessummary.totalques - $rootScope.quessummary.answered;  
