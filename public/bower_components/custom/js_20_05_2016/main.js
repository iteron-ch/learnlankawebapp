...
app.controller('MainCtrl', function ( $scope, $rootScope, $location, $filter, $sce, $timeout, $route, $modal ) {
    ...
    $scope.updateDropdownValues = function( json ){
       ...
       switch (json.type) {
            ...
            case 'strand':
                //var filterStrandCode3333 = $filter('getStrandById')($scope.MASTERS.strand[$scope.selected.subject], $scope.selected.strand);
            ...
            case 'substrand':
                var filterStrandCode = $filter('filter')($scope.MASTERS.strand[$scope.selected.subject], {id:$scope.selected.strand},true)[0];
                var filterSubStrandCode = $filter('filter')($scope.MASTERS.substrand[$scope.selected.strand], {id:$scope.selected.substrand},true)[0];
...
app.filter('getStrandById', function() {
  return function(strands, id) {
        $.each(strands,function(i,strand){
            if(parseInt(strand.id) == parseInt(id)){
                console.log(strands[i]);
                return strands[i];
            }
        });
  };
