...
/**
 * @ngdoc function
 * @name questionBuilder.controller:multipleChoiceCtrl
 * @description
 * # multipleChoiceCtrl
 * Controller of the questionBuilder
 */
var app = angular.module('questionBuilder');
app.controller( 'multipleChoiceCtrl' , function ( $scope, $window, $sce, $rootScope, $routeParams, $modal, Flash, $http, $timeout ) {
            ...
            questions: [
                {
                    ...
                    correctAns: [{ischeck:false},{ischeck:false},{ischeck:false}],
    ...
    $scope.addOption = function (parentIndex) {
        ...
        $scope.questionJSON.questions[parentIndex].correctAns.push({ischeck:false});
    ...
    $scope.removeOption = function (parentIndex, optionIndex) {
        ...
        if ($scope.questionJSON.questions[parentIndex].option.length > 2) {
            ...
            angular.forEach($scope.questionJSON.questions[parentIndex].option, function (ele, index) {                        
                var optionName = 'option' + (parentIndex + 1) + (index + 1);
                $timeout(function (){
                    CKEDITOR.inline(optionName, {extraPlugins: 'mathjax'});
                });
            });
    ...
    $scope.submitMultipleCQ = function ( navigateTo, actionMode ) {
    ...
    $scope.previewQuestion = function (preQuesIndex, popupSize) {
        ...
        if ($scope.validatedVal) {
            ...
            var modalInstance = $modal.open({
                templateUrl: 'multipleQuestionPre',
                controller: 'questionPreMCCtrl',
                size: popupSize,
                scope: $scope,
                resolve: {
...
app.controller('questionPreMCCtrl', function ( $scope, $modalInstance, questionPre, $http, $modal ) {
