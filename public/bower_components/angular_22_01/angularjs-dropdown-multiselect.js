    ...
    return {
        ...
        template:
                "<div class='btn-group' data-ng-class='{open: open}'>" +
                    "<button class='btn btn-small dropdown-toggle' data-ng-click='openDropdown()'>Select Reason</button>" +
        ...
        controller: function ($scope) {
            ...
            $(window).click(function(event) { 
                if(!$(event.target).hasClass('dropdown-toggle') && !$(event.target).hasClass('glyphicon-ok') && !$(event.target).hasClass('glyphicon-remove') && !$(event.target).is("a")){
                    $scope.open = false;
                    $scope.$digest();
                }
            });
