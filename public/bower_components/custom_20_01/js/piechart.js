...
/**
 * @ngdoc function
 * @name questionBuilder.controller:piechartCtrl
 * @description
 * # piechartCtrl
 * Controller of the questionBuilder
 */
var app = angular.module('questionBuilder');
app.controller( 'piechartCtrl' , function ( $scope, $window, $sce, $rootScope, $routeParams, $modal, Flash, $http, $timeout ) {
    ...
    $scope.submitpiechartCQ = function ( navigateTo, actionMode ) {
    ...
    $scope.setAnswer = function (index) {
        ...
        if ($scope.validatedVal) {
            ...
            $scope.percent = $scope.questionJSON.questions[index].correctAns[0].value;
        } else {
    ...
    $scope.update = function( val, parentIndex ){
        $scope.percent = val !== '' ? val : 0;
        $scope.questionJSON.questions[parentIndex].correctAns[0].value = $scope.percent;
    };
    ...
    $scope.previewQuestion = function (preQuesIndex, popupSize) {
        ...
        if ($scope.validatedVal) {
            ...
            var modalInstance = $modal.open({
                templateUrl: 'piechartPre',
                controller: 'piechartPreCtrl',
                size: popupSize,
                scope: $scope,
                resolve: {
...
app.controller('piechartPreCtrl', function ( $scope, $modalInstance, questionPre, $http, $modal ) {
    ...
    $scope.percentpre = 0;
    $scope.updatepre = function( val, parentIndex ){
        $scope.percentpre = val;
    };
