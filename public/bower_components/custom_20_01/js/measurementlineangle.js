/**
 * Created by sunny.rana on 08-12-2015.
 */
'use strict';

/**
 * @ngdoc function
 * @name questionBuilder.controller:measurementlineangleCtrl
 * @description
 * # measurementlineangleCtrl
 * Controller of the questionBuilder
 */
var app = angular.module('questionBuilder');
app.controller( 'measurementlineangleCtrl' , function ( $scope, $window, $sce, $rootScope, $routeParams, $modal, Flash, $http, $timeout ) {
    ...
    if( $scope.questionData ) {
        ...
        $scope.questionJSON = {
            ...
            questions: [
                {
                    ques: '',
                    option: {value: '', protractor:'', noofoption: 1},
                    correctAns: [{label: '', value: ''}],
    ...
    $scope.imageBind = function( questionIndex, action ){
        if( action === 'ques' ){
            if( $scope.questionJSON.id && $scope.questionJSON.questions[questionIndex].option.value ){
                return $sce.trustAsHtml('<img src="/questionimg/'+$scope.questionJSON.questions[questionIndex].option.value+'"/>');
            } else {
                return '';
            }
        } else {
            if( $scope.questionJSON.id && $scope.questionJSON.questions[questionIndex].option.protractor ){
                return $sce.trustAsHtml('<img src="/questionimg/'+$scope.questionJSON.questions[questionIndex].option.protractor+'"/>');
            } else {
                return '';
            }
        }
    ...
    $scope.submitmeasurementlineangleCQ = function ( navigateTo, actionMode ) {
    ...
    $scope.setAnswer = function (index) {
        ...
        if ($scope.validatedVal) {
            ...
            var canvas = new fabric.Canvas('c'+index);            
            
            if($scope.questionJSON.questions[index].option.protractor){
                tempprotractor = '/questionimg/'+$scope.questionJSON.questions[index].option.protractor;
            }else{
    ...
    $scope.editQuestion = function () {
        ...
        angular.forEach($scope.questionJSON.questions, function (ele, index) {
            ...
            if($('#setanswer'+index).find('.canvas-container').length){
                $('#setanswer'+index).find('.canvas-container').remove();
                $('#setanswer'+index).html('<canvas id="c'+index+'" width="500" height="400"></canvas>');
            } 
...
//                $scope.questionJSON.questions[index].option.value = $('#imageID' + index).attr('ng-src');
            ...
            var modalInstance = $modal.open({
                templateUrl: 'measurementlineanglePre',
                controller: 'measurementlineanglePreCtrl',
                size: popupSize,
                scope: $scope,
                resolve: {
    ...
    $scope.bindClickProtractor = function(e){
        var quesId = $(e).attr('id').split('_')[1];
        var data = new FormData();
        var file = typeof $(e)[0].files[0] === 'undefined' ? '' : $(e)[0].files[0];
        data.append('file', file);
        data.append('dimension', [280,160]);
        data.append('selectedImg', $scope.questionJSON.questions[quesId].option.protractor);
        $.ajax({
            ...
            success: function( data, textStatus, jqXHR ){
                $scope.questionJSON.questions[quesId].option.protractor = data.filename;
                $scope.$digest();
...
app.controller('measurementlineanglePreCtrl', function ( $scope, $modalInstance, questionPre, $http, $modal, $timeout ) {
    ...
    $timeout(function(){
        var canvas = new fabric.Canvas('canswer'+$scope.quesIndex);
        if($scope.quesArray.questions[$scope.quesIndex].option.protractor){
            tempprotractor = '/questionimg/'+$scope.quesArray.questions[$scope.quesIndex].option.protractor;
        }else{
            tempprotractor = '../../images/protractor.gif';
        }
        fabric.Image.fromURL(tempprotractor, function(oImg) {
